...
import { Plus, Check, AlertCircle, Pencil, Trash, Play, Pause, HardDrive, DollarSign, Layers } from 'lucide-react'

export default function StoragePackagesManager() {
  ...
  const fetchPackages = async () => {
    try {
      ...
      console.error(err)
      setError('فشل تحميل باقات التخزين.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingPkg) {
        await API.put(`/admin/storage-packages/${editingPkg.id}`, payload)
        showToast('تم تحديث حزمة التخزين بنجاح.', 'success')
      } else {
        await API.post('/admin/storage-packages', payload)
        showToast('تم إنشاء حزمة التخزين بنجاح.', 'success')
      }
      setIsFormOpen(false)
      fetchPackages()
    } catch (err: any) {
      console.error(err)
      showToast(err.response?.data?.message || 'فشل حفظ حزمة التخزين.', 'error')
    }
  }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await API.delete(`/admin/storage-packages/${id}`)
      showToast('تم حذف حزمة التخزين بنجاح.', 'success')
      fetchPackages()
    } catch (err: any) {
      console.error(err)
      showToast('فشل حذف حزمة التخزين.', 'error')
    }
  }

  const handleToggleStatus = async (id: number) => {
    try {
      await API.post(`/admin/storage-packages/${id}/toggle`, {})
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-slate-900/40 p-4 border border-slate-800 rounded-2xl">
        <div>
          <h2 className="text-sm font-black text-slate-200">إدارة باقات مساحة التخزين الإضافية</h2>
          <p className="text-[10px] text-slate-400 mt-1">تحديد أسعار وقيم المساحات الإضافية المتاحة للشراء من المعلمين</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="text-center py-10 border-2 border-dashed border-slate-800 rounded-2xl text-slate-500 text-xs">
          لا توجد باقات تخزين إضافية حالياً.
        </div>
      ) : (
          ...
          <table className="w-full text-right text-xs">
            <thead className="bg-slate-900/60 text-slate-400 border-b border-slate-800 font-bold">
              <tr>
                ...
                <th className="p-3.5">حجم التخزين (GB)</th>
                <th className="p-3.5">السعر الشهري</th>
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-350">
              {packages.map((pkg) => (
                <tr key={pkg.id} className="hover:bg-slate-900/10 transition-colors">
                  ...
                  <td className="p-3.5 font-medium">{pkg.storage_gb} GB</td>
                  <td className="p-3.5 font-black text-emerald-400">{Number(pkg.price).toFixed(2)} ج.م / شهر</td>
                  <td className="p-3.5">
                    {pkg.active ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-4 text-right" dir="rtl">
            <h3 className="text-base font-black text-slate-200 flex items-center gap-2">
              ...
              <span>{editingPkg ? 'تعديل حزمة تخزين إضافية' : 'إنشاء حزمة تخزين إضافية جديدة'}</span>
            </h3>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-[10px] text-slate-400 font-bold block mb-1">اسم الباقة (مثال: +10 جيجا)</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] text-slate-400 font-bold block mb-1">المساحة الإضافية (GB)</label>
                  <input
                    type="number"
                    required
                    min={1}
                ...
                <div>
                  <label className="text-[10px] text-slate-400 font-bold block mb-1">السعر الشهري للباقة (ج.م)</label>
                  <input
                    type="number"
                    required
                    min={1}
